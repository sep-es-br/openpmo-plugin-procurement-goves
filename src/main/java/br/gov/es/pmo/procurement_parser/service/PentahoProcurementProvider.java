...
@Component public class PentahoProcurementProvider implements IProcurementProvider {
 ...
 public ProcurementDto getProcurement(Long id){return query(props.getProcurementDetailPath(),props.getProcurementDetailDataAccessId(),new String[][]{{"paramcod_lic",id.toString()}}).getResultset().stream().findFirst().map(this::detail).orElse(null);}
